# PyTestConnection - Avaliador de Qualidade de Internet v{{VERSION}}

## Sobre o Programa
O PyTestConnection é uma ferramenta robusta e confiável desenvolvida em Python para medir e avaliar a qualidade da sua conexão de rede (Download, Upload, Ping e Jitter). A aplicação faz uso de múltiplos motores de teste (Speedtest e Cloudflare) garantindo um diagnóstico altamente acurado.
...
> Para utilizar ou mover a aplicação sem perder seu histórico ou ícones, não mova apenas o `.exe`. Mova a pasta `dist` inteira. Mais especificamente, mantenha o `.exe` acompanhado das pastas `config`, `data` e `src` geradas.

---

## O Que a Aplicação Mostra na Tela?

- **Medidores e Gráficos:** Informações centralizadas informando os dados obtidos de Download, Upload, ICMP Ping e Jitter. Há um gráfico preenchido dinamicamente demonstrando o panorama.
...
- **Semáforo de Adequação:** Logo ao lado direito, o programa lhe entrega um veredito real usando luzes de semáforo (Verde, Amarelo e Vermelho) para analisar se sua rede suportará: Videochamadas, Navegação Básica, Transferência de Arquivos, Jogos Online e IP-TV (Streaming 4K).
